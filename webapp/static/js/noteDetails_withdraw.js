    ...
    function webInit() {
        ...
        akAjax('get','/akadmin/ntc/ntcDetailQuery?bsnJrnlNo='+bsnJrnlNo,'',function(data){
        	if (data && data.state === 1) {
                ...
                infForm.payeeAccount.value = data.ntc.rcvPymtPsAccno;
                infForm.payeeAccountName.value = data.ntc.rcvPymtPsAccnm;
                infForm.recipientBank.value = data.ntc.rpyBkNm;
                infForm.recipientBankID.value = data.ntc.rpyBkAccNo;

                $seePics.prop('href', $seePics.data('akhref') + '?goodsNum=' + data.ntc.gdsId);
            }
		});
    }
